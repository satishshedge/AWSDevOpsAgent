...
The original agent templates live in [`custom-agents-v1/`](custom-agents-v1/)
and fall into two groups. Deploy any of them, mix and match freely.

> Clone the repo and `cd custom-agents-v1` before running the commands below.
...
To see every deployable agent key, run `./deploy.sh --list` from inside this
folder.
...
![Cost optimization custom agents listed in the AWS DevOps Agent web app after deployment](custom-agents-v1/Custom_Agent.png)
